  ...
  var sortableParameters = {
    ...
    stop: function(event, ui) {
      ...
      /*if (item.hasClass('item')) {
        var tmp = $('<p contenteditable="true">SWAG</p>');
  ...
  iframe.on('load', function() {
    ...
    $('.items div').draggable({
      ...
      drag: function(event, ui) {
        var target = $(allElementsFromPointIframe(event.clientX, event.clientY, iframe.offset().left, iframe.offset().top));
        var div = $(target).filter('div').not($(this));
        selected.css({
          'border': 'none'
        });
      ...
      stop: function(event, ui) {
        ...
        item = iframe.contents().find('#highlight').replaceWithPush(element);
        
        console.log("EVENT",event);
        console.log("UI",ui);
        //item = element;//iframe.contents().find(event.target);
        
        console.log("ITEM",item);
        console.log("ITEM PARENT",item.parent());
        console.log("SENDER PARENT",sender.parent());
        
        receiver = item.parent();
        console.log("RECEIVER:", item.parent());
        //Create wrapper around items passed as new cells
        /*if (item.hasClass('item')) {
        ...
        if (!item.hasClass('sortable-cell')) {
          console.log("Wrap new cell");
          //item.replaceWith(element);
